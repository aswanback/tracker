...
class AccelStepper:
    ...
    def digitalWrite(self, pin:int, iovalue:int):
        # pi_iovalue = GPIO.HIGH if iovalue == IOValue.HIGH.value else GPIO.LOW
    ...
    def setOutputPins(self, mask):
        ...
        for i in range(numpins):
            if mask & (1 << i):
                self.digitalWrite(self._pin[i], IOValue.HIGH.value ^ self._pinInverted[i])
            else:
                self.digitalWrite(self._pin[i], IOValue.LOW.value ^ self._pinInverted[i])
    def step0(self, step):
        if self._speed > 0:
            self._forward()
        else:
    ...
    def disableOutputs(self):
        ...
        if self._enablePin != 0xff:
            self.pinMode(self._enablePin, IOType.OUTPUT.value)
            self.digitalWrite(self._enablePin, IOValue.LOW.value ^ self._enableInverted)

    def enableOutputs(self):
        if not self._interface:
            return
        self.pinMode(self._pin[0], IOType.OUTPUT.value)
        self.pinMode(self._pin[1], IOType.OUTPUT.value)
        if self._interface == MotorInterfaceType.FULL4WIRE or self._interface == MotorInterfaceType.HALF4WIRE:
            self.pinMode(self._pin[2], IOType.OUTPUT.value)
            self.pinMode(self._pin[3], IOType.OUTPUT.value)
        elif self._interface == MotorInterfaceType.FULL3WIRE or self._interface == MotorInterfaceType.HALF3WIRE:
            self.pinMode(self._pin[2], IOType.OUTPUT.value)
        if self._enablePin != 0xff:
            self.pinMode(self._enablePin, IOType.OUTPUT.value)
            self.digitalWrite(self._enablePin, IOValue.HIGH.value ^ self._enableInverted)
